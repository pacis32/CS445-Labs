const SERVER_ROOT = "http://localhost:3000/api/music";
...
async function fetchAllSongs() {
    ...
    <th scope="col-auto">Release Date</th>
    <th scope="col-auto">Action</th>
    </tr>
    `;
    // loop to access all rows
    for (let r of musicList) {
        tab += `
        <tr>
        <td>${r.id}</td>
        <td>${r.title}</td>
        <td>${r.releaseDate}</td>
        <td>
        <button id='add' class="test" " onclick="addToPlayList('${r.id}')">+</button>
        </td>
        </tr>
...
async function getPlayList() {
    ...
    for (let m of playlist) {
     ...
     <td>${m.orderId}</td>
     <td>${m.title}</td>
     <td><button id='deletebtn' onclick="removeFromPlayList('${m.songId}')">-</button>
     </td>
     <td><button class='play' data-num22:"${m.urlPath}">play</button>
     </td>
    </tr>`
    }
...
function searchSong(){
 ...
 let searchSong=document.getElementById('searchBar').value;
   console.log(searchSong)
    fetch('http://localhost:3000/api/music?search= searchSong',{
      ...
      .then(data=>{
        console.log(data);      
            ...
            document.getElementById('dtable').innerHTML='searchSong'; 
